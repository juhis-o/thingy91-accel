...
typedef int (*sensor_fifo_raw_get_t)(const struct device *dev, int16_t *val);
...
__subsystem struct sensor_driver_api {
	...
	sensor_fifo_raw_get_t fifo_get;
};
...
static inline int sensor_fifo_get(const struct device *dev, int16_t *val){
	const struct sensor_driver_api *api =
		(const struct sensor_driver_api *)dev->api;
	return api->fifo_get(dev, val);
}

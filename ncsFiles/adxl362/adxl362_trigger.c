...
static void adxl362_thread_cb(const struct device *dev)
{
	...
	if (drv_data->fifo_handler != NULL) {
		drv_data->fifo_handler(dev, drv_data->fifo_trigger);
	}
...
static void adxl362_thread_cb_fifo(const struct device *dev){
	struct adxl362_data *drv_data = dev->data;
	LOG_INF("Trigger_CB_FIFO: %d %d",drv_data->fifo_trigger->type,drv_data->fifo_trigger->chan);
	drv_data->fifo_handler(dev, drv_data->fifo_trigger);
}
...
static void adxl362_work_cb(struct k_work *work)
{
	...
	adxl362_thread_cb_fifo(drv_data->dev);
}
#endif
			...
			sensor_trigger_handler_t handler)
{
	...
	case SENSOR_TRIG_FIFO:
		k_mutex_lock(&drv_data->trigger_mutex, K_FOREVER);
		drv_data->fifo_handler = handler;
		drv_data->fifo_trigger = trig;
		LOG_INF("Trigger: %d %d\n DRV_DATA: %d, %d",trig->type,trig->chan, drv_data->fifo_trigger->type, drv_data->fifo_trigger->chan);
		k_mutex_unlock(&drv_data->trigger_mutex);

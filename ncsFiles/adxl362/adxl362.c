...
static int adxl362_fifo_cmd(const struct device *dev, void *data, size_t length){
	//Laitteen asetukset
	const struct adxl362_config *cfg = dev->config;
	//FIFO-puskurin luku käsky
...
//RX-puskuri jossa on yksi tyhjä paikka alussa, koska käskyn antaminen ja lukeminen tapahtuvat samaan aikaan. Tämän vuoksi jäisi yksityhjä taulukon alkuun. Asetamme määrän, kuinka paljon haluamme lukea funktion parametreissä.
				...
				enum sensor_channel chan)
{
	...
	ret = adxl362_fifo_cmd(dev, buf,sizeof(buf));
	if (ret) {
		return ret;
	}

	for(uint16_t i = 0,conv_i = 0; i < 510;i++,conv_i+=2){
		fifo[i] = ((buf[conv_i+1] & 0x3f) << 8) | buf[conv_i];
		data->acc_xyz[i] = sys_le16_to_cpu(((fifo[i] & 0x3000) << 2) | fifo[i]);
	}
...
static int adxl362_fifo_get(const struct device *dev,int16_t *val){
	struct adxl362_data *data = dev->data;
	memcpy(val,data->acc_xyz,1020);
	return 0;
};
...
static int adxl362_chip_init(const struct device *dev)
{
	...
	ret = adxl362_fifo_setup(dev, ADXL362_FIFO_STREAM, 253, false);
	if (ret) {
		return ret;
	}
